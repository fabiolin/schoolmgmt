...
BEGIN
    ...
    _permanentAddress := COALESCE(data->>'permanentAddress', NULL);
    _fatherName := COALESCE(data->>'fatherName', NULL);
    _fatherPhone := COALESCE(NULLIF(data->>'fatherPhone', ''), NULL);
    _motherName := COALESCE(NULLIF(data->>'motherName', ''), NULL);
    _motherPhone := COALESCE(NULLIF(data->>'motherPhone', ''), NULL);
    ...
    _systemAccess := COALESCE(NULLIF(data->>'systemAccess', '')::BOOLEAN, NULL);
    _className := COALESCE(NULLIF(data->>'class', ''), NULL);
    _sectionName := COALESCE(NULLIF(data->>'section', ''), NULL);
    _admissionDt := COALESCE(NULLIF(data->>'admissionDate', '')::DATE, NULL);
    _roll := COALESCE(NULLIF(data->>'roll', '')::INTEGER, NULL);
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN QUERY
            SELECT _userId::INTEGER, false, 'Unable to ' || COALESCE(_operationType, 'process') || ' student', SQLERRM;
END;
$BODY$;
