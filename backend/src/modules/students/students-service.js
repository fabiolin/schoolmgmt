...
const addNewStudent = async (payload) => {
    ...
    const result = await addOrUpdateStudent(sanitized);
    if (!result.status) {
        throw new ApiError(500, result.message || result.description || "Unable to add student");
    }
...
const updateStudent = async (payload) => {
    ...
    const result = await addOrUpdateStudent(sanitized);
    if (!result.status) {
        throw new ApiError(500, result.message || result.description || "Unable to update student");
    }
